...
void MIEServerMT::addDoc(int newsockfd) {
    ...
    receiveDoc(newsockfd, &id, &mat, &keywords, true);
...
void MIEServerMT::receiveDoc(int newsockfd, int* id, Mat* mat, vector<vector<unsigned char> >* keywords, bool keepData) {
...
//    socketReceive(newsockfd, data, 5*sizeof(int));        //without zip
    ...
    *id = readIntFromArr(data, &pos);
    ...
    if(keepData){
        ...
        int tpos = pos;
        int img_size = readIntFromArr(data, &pos);
        int txt_size = readIntFromArr(data, &pos);

        char name[500];
        sprintf(name, "%sData/Server/MIE/%d.data", homePath.c_str(), *id);
        FILE* file = fopen(name, "wb");
        fwrite(data+tpos, 1, img_size+txt_size+2*sizeof(int), file);
        fclose(file);
        pos+=img_size+txt_size;
    }
...
bool MIEServerMT::readOrPersistFeatures() {
    ...
    if (nImgs == 0 && f != NULL) {
        ...
        int featureSize = readIntFromArr(buff, &pos);
...
//        int nImgs = (int)imgFeatures.size();
        ...
        size_t buffSize = featureSize*sizeof(float);
        char* buff2 = (char*)malloc(buffSize);
        printf("allocced %d %d %d\n", buffSize, featureSize, sizeof(float));
        bzero(buff2,buffSize);
        imgFeaturesLock.lock();
    ...
    if (nDocs == 0 && f != NULL) {
        ...
        char buff[2*sizeof(int)];
        fread(buff, 1, 2*sizeof(int), f);
        nDocs = readIntFromArr(buff, &pos);
        keywordSize = readIntFromArr(buff, &pos);
        for (int i = 0; i < nDocs; i++) {
            fread(buff, 1, 2*sizeof(int), f);
            pos = 0;
            int docId = readIntFromArr(buff, &pos);
            nKeywords = readIntFromArr(buff, &pos);
        ...
        textFeaturesLock.lock();
        int keywordSize = -1;
        map<int,vector<vector<unsigned char> > >::iterator it=textFeatures.begin();
        while(keywordSize == -1 && it != textFeatures.end()){
            if(!it->second.empty()){
                keywordSize = (int)(it->second)[0].size();
            }
            else{
                ++it;
            }
...
bool MIEServerMT::readIndex() {
    ...
    if (f1 != NULL) {
        ...
        nImgs = readIntFromArr(buff, &pos);
        indexSize = readIntFromArr(buff, &pos);
        ...
        for (int i = 0; i < indexSize; i++) {
            ...
            for (int j = 0; j < postingListSize; j++) {
                int docId=0, score=0;
                docId = readIntFromArr(buff2, &pos);
                score = readIntFromArr(buff2, &pos);
                imgIndexLock.lock();
...
void MIEServerMT::search(int newsockfd) {
    ...
    sendQueryResponse(newsockfd, &imgResults/*&mergedResults*/, 20);
}

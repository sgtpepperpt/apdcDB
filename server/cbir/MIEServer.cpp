...
void MIEServer::receiveDoc(int newsockfd, int& id, Mat& mat, vector<vector<unsigned char> >& keywords, bool keepData) {
...
void MIEServer::sendDoc(int newsockfd){
    char buff[sizeof(uint32_t)];
    socketReceive(newsockfd, buff, sizeof(uint32_t));
    int pos = 0;
    int id = readIntFromArr(buff, &pos);
    
    char fname[120];
    memset(fname, 0, 120);
    sprintf(fname, "%sData/Server/MIE/%d.data", homePath.c_str(), id);
    ...
    fclose(file);
    ///send data
    zipAndSend(newsockfd, buffer, fsize);
    free(buffer);
}